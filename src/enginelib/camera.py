...
class ListenerCamera(BaseCamera):
    listener: openal.Listener

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.listener = openal.oalGetListener()
        self.game.add_callback('on_frame', self.update_listener_vars)

    @script.every_n_ms(16, always_fire=True)
    def update_listener_vars(self, _delta_t):
        self.listener.set_position(self.position)
        self.listener.set_orientation((*self.front, *self.up))
...
class InputCamera(BasicCamera, ListenerCamera):
    ...
    def __init__(self, game):
        ...
        self.game.add_callback('on_cursor_pos_update', self.handle_cursor_pos, always_fire=True)
